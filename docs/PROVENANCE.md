...
[文档导航](README.md) · [开发指南](DEVELOPMENT.md)

- 上游：[scutclient/luci-app-scutclient](https://github.com/scutclient/luci-app-scutclient)
...
安装检测改用 Lua 模式解析，避免 sed 的多层转义；fetch 增加 HTTP 错误显示、textarea.value 和串行轮询。这些整理差异需要上机回归，当前证据见 [验证状态](VALIDATION.md)。

构建规则参考 [ImmortalWrt luci.mk](https://github.com/immortalwrt/luci/blob/master/luci.mk)，Makefile 引用 SDK 的 feeds/luci/luci.mk，适合独立放入 package/luci-app-scutclient。最终以对应 SDK 为准。
